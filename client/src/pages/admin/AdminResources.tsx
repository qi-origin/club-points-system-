...
import { Table, Button, Space, Tag, Modal, Form, Input, InputNumber, message, Typography } from 'antd';
...
export default function AdminResources() {
  ...
  const handleSave = async (values: any) => {
    try {
      if (editingResource) {
        await adminApi.updateResource(editingResource.id, values);
        message.success('更新成功');
      } else {
        await adminApi.createResource(values);
  ...
  const handleToggleStatus = async (record: any) => {
    ...
    try {
      await adminApi.toggleResourceStatus(record.id, newStatus);
  ...
  const columns = [
    ...
    { title: '总库存', dataIndex: 'totalStock', width: 100 },
    {
      ...
      render: (s: number) => <Tag color={s === 1 ? 'green' : 'default'}>{s === 1 ? '上架' : '下架'}</Tag>,
    },
    {
      title: '操作', width: 220,
  ...
  return (
    <div>
      ...
      <Button type="primary" icon={<PlusOutlined />} onClick={handleCreate} style={{ marginBottom: 16 }}>新增资源</Button>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSave}>
          ...
          <Form.Item name="imageUrl" label="图片URL（可选）">

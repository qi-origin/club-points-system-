...
export default function AdminStudents() {
  ...
  const fetchData = () => {
    ...
    adminApi.getStudents({ page, pageSize: 20, keyword: keyword || undefined })
      .then((res) => { setStudents(res.data.list); setTotal(res.data.total); })
      .finally(() => setLoading(false));
  };

  useEffect(() => { fetchData(); }, [page]);

  const handleCreate = () => { setEditingStudent(null); form.resetFields(); setModalOpen(true); };
  const handleEdit = (record: any) => { setEditingStudent(record); form.setFieldsValue(record); setModalOpen(true); };

  const handleSave = async (values: any) => {
    try {
      if (editingStudent) { await adminApi.updateStudent(editingStudent.id, values); message.success('更新成功'); }
      else { await adminApi.createStudent(values); message.success('创建成功'); }
  ...
  const handleToggleStatus = async (record: any) => {
    ...
    try { await adminApi.toggleStudentStatus(record.id, newStatus); message.success(newStatus === 1 ? '已启用' : '已禁用'); fetchData(); }
    catch (err: any) { message.error(err.response?.data?.error || '操作失败'); }
  };

  const handleManualPoints = (record: any) => { setSelectedStudent(record); manualForm.resetFields(); setManualModalOpen(true); };

  const handleManualSubmit = async (values: any) => {
    try { await adminApi.manualPoints(selectedStudent.id, values); message.success('操作成功'); setManualModalOpen(false); fetchData(); }
    catch (err: any) { message.error(err.response?.data?.error || '操作失败'); }
  };
  ...
  const columns = [
    ...
    { title: '状态', dataIndex: 'status', width: 80, render: (s: number) => <Tag color={s === 1 ? 'green' : 'red'}>{s === 1 ? '正常' : '禁用'}</Tag> },
    ...
    { title: '注册时间', dataIndex: 'createdAt', width: 170, render: (d: string) => new Date(d).toLocaleString() },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          ...
          <Button size="small" onClick={() => handleManualPoints(record)}
            style={{ color: '#f59e0b', borderColor: 'rgba(245,158,11,0.3)' }}>加减分</Button>
  ...
  return (
    <div>
      <div className="game-title" style={{ marginBottom: 20, fontSize: 18 }}>
        <TeamOutlined style={{ marginRight: 8 }} />学生管理
      </div>
      <Space style={{ marginBottom: 16 }}>
        <Input prefix={<SearchOutlined />} placeholder="搜索姓名/学号" value={keyword}
          onChange={(e) => setKeyword(e.target.value)} onPressEnter={() => { setPage(1); fetchData(); }}
          style={{ width: 200 }} />
      ...
      <Modal title={editingStudent ? '编辑学生' : '新增学生'} open={modalOpen}
      ...
      <Modal title={`手动调整积分 - ${selectedStudent?.name}`} open={manualModalOpen}
        onCancel={() => setManualModalOpen(false)} onOk={() => manualForm.submit()}>
